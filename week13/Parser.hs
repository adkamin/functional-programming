...
space1 :: Parser ()
space1 = sat isSpace *> space

endBy :: Parser a -> Parser b -> Parser [a]
-- p `endBy` sep = p `sepBy` sep <* sep      -- Returns nothing if it cannot further parse
p `endBy` sep = many (p <* sep)              -- Fixed

atMost :: Int -> Parser a -> Parser [a]
atMost 0 p = pure []
atMost n p = pure (:) <*> p <*> atMost (n-1) p <|> pure []
